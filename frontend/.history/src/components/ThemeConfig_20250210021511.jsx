...
export const ThemeSelector = ({ currentTheme, onThemeChange, currentFont, onFontChange, currentAccentColor, onAccentColorChange }) => {
  ...
  useEffect(() => { const handleClickOutside = (event) => { if (!event.target.closest('.menu-container')) setFontMenuOpen(false), setColorMenuOpen(false); }; document.addEventListener('click', handleClickOutside); return () => document.removeEventListener('click', handleClickOutside); }, []);

  return (
    <div className="relative p-4 bg-white shadow-lg rounded-lg">
      <div className="flex justify-between items-center mb-4">
        <label className="text-sm font-semibold text-gray-800">Select Theme</label>
        <div className="flex items-center space-x-4">
          <div className="flex items-center space-x-3 cursor-pointer bg-gray-100 p-2 rounded-lg shadow-md hover:bg-gray-200 menu-container" onClick={() => setFontMenuOpen(!fontMenuOpen)}>
            <span className={`${currentFont} text-base font-semibold`}>A</span><ChevronDown size={18} className="text-gray-700" />
          </div>
          <div className="flex items-center space-x-3 cursor-pointer bg-gray-100 p-2 rounded-lg shadow-md hover:bg-gray-200 menu-container" onClick={() => setColorMenuOpen(!colorMenuOpen)}>
            <div className={`w-7 h-7 rounded-full border ${currentAccentColor}`} /><ChevronDown size={18} className="text-gray-700" />
          </div>
        </div>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        {Object.entries(resumeThemes).map(([key, theme]) => (
          <button key={key} onClick={() => onThemeChange(key)} className={`relative p-3 rounded-lg border text-sm font-medium transition-all duration-200 hover:shadow-md ${currentTheme === key ? 'border-accent bg-accent text-white shadow-md' : 'border-gray-300 hover:border-gray-400 text-gray-700'}`}>{theme.name}{currentTheme === key && <Check size={14} className="absolute top-1 right-1 text-white" />}</button>
        ))}
      </div>
      {fontMenuOpen && (
        <div className="absolute top-full mt-2 left-0 w-48 bg-white border rounded-lg shadow-lg p-3 space-y-4 menu-container">
          <label className="block text-xs font-semibold text-gray-700">Font</label>
          <div className="grid grid-cols-2 gap-2">
            {fontOptions.map(font => (
              <button key={font.value} onClick={() => onFontChange(font.value)} className={`relative p-2 rounded-lg border text-xs font-medium transition-all duration-200 hover:shadow-md ${currentFont === font.value ? 'border-accent bg-accent text-white' : 'border-gray-300 hover:border-gray-400 text-gray-700'} ${font.value}`}>{font.name}{currentFont === font.value && <Check size={12} className="absolute top-1 right-1 text-white" />}</button>
            ))}
          </div>
        </div>
      )}
        ...
        <div className="absolute top-full mt-2 left-0 w-48 bg-white border rounded-lg shadow-lg p-3 space-y-4 menu-container">
          ...
          <div className="flex space-x-2">
            {accentColorOptions.map(color => (
              <button key={color.value} onClick={() => onAccentColorChange(color.value)} className={`w-7 h-7 rounded-full border-2 transition-all duration-200 hover:shadow-md ${color.value} ${currentAccentColor === color.value ? 'border-accent shadow-md' : 'border-gray-300'}`} />
            ))}
          </div>
        </div>
      )}
