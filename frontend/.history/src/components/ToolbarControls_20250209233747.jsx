          ...
          >
            ...
            {currentTheme === key && (
              <span className="absolute top-1 right-1">
                <Check size={12} className="text-accent-600 dark:text-accent-400" />
              </span>
            )}
          </button>
        ))}
...
export const ThemeSelectorDropdown = ({ currentTheme, onThemeChange }) => {
  return (
    <select
      ...
      className="w-full p-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 focus:ring-2 focus:ring-accent-500 dark:focus:ring-accent-400"
    >
      {Object.entries(resumeThemes).map(([key, theme]) => (
