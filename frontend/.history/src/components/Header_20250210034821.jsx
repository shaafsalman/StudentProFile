...
import { Layout, X, CheckCircle2 } from 'lucide-react';
...
const AdvancedSidebarMenu = ({ 
  ...
  const sections = [
    ...
    },
    {
      ...
      renderOption: (option) => (
        <div className="flex items-center justify-between w-full">
          <span className={`${option.value} text-sm font-medium`}>{option.name}</span>
          <span className={`${option.value} text-xs text-gray-500 dark:text-gray-400 opacity-70`}>
            Aa
          </span>
        </div>
      )
      ...
      renderOption: (option) => (
        <div className="flex items-center gap-3 w-full justify-between">
          <div className="flex items-center gap-3">
            <div 
              className={`w-6 h-6 rounded-full border-2 ${option.value}`} 
            />
            <span className="text-sm font-medium">{option.name || 'Accent Color'}</span>
          </div>
        </div>
      )
    }
  ...
  return (
    <div className="fixed top-16 left-0 bottom-0 w-[35%] bg-white dark:bg-gray-900 z-[99] shadow-2xl border-r border-gray-200 dark:border-gray-800 transform transition-transform duration-300 ease-in-out">
      {/* Header with Close Button */}
      <div className="flex justify-between items-center p-6 border-b dark:border-gray-800 bg-gray-50 dark:bg-gray-950">
        <div>
        ...
        <button 
          onClick={onClose}
          className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 transition-colors bg-gray-100 dark:bg-gray-800 p-2 rounded-full"
        >
          <X size={20} />
        </button>
      </div>
        ...
        <div className="w-24 border-r dark:border-gray-800 bg-gray-50 dark:bg-gray-950">
          {sections.map((section) => (
            <button
              ...
              className={`
                w-full py-4 text-center border-b dark:border-gray-800 relative
            ...
            >
              {section.title}
              {activeSection === section.id && (
                <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 h-1 w-8 bg-accent rounded-full" />
              )}
            </button>
          ))}
        </div>
                  ...
                  {section.options.map((option) => (
                    ...
                    >
                      ...
                      {section.currentValue === option.value && (
                        <CheckCircle2 
                          size={20} 
                          className="text-accent group-hover:scale-105 transition-transform" 
                        />
                      )}
                    </button>
                  ))}
