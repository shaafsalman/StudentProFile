...
import { useTheme } from "../../../../ShaafSalmanWeb/frontend/src/context/ThemeContext";
...
const ThemeToggleButton = () => {
  ...
  return (
    ...
    >
      ...
      {theme === "dark" ? (
        <Sun
          className="w-6 h-6 text-white transition-all duration-300 ease-in-out transform group-hover:rotate-180 group-hover:text-white"
        />
      ) : (
        <Moon
          className="w-6 h-6 text- transition-all duration-300 ease-in-out transform group-hover:rotate-180 group-hover:text-white"
        />
      )}
    </button>
  );
