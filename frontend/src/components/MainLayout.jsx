...
const MainLayout = () => {
  ...
  return (
    <div className="h-screen w-screen overflow-hidden flex">
      ...
      {/* Right: Preview + Theme Selector */}
      <div className={`h-full transition-all duration-300 relative ${isEditorCollapsed ? "w-full" : "w-[65%]"}`}>
        <div className="absolute top-0 left-0 right-0 z-10 px-6 py-4 bg-background-light dark:bg-background-dark flex items-center gap-4">
          {isEditorCollapsed && (
            <Button variant="ghost" size="sm" icon={PanelLeft} onClick={() => setIsEditorCollapsed(false)}>
              Show Editor
            </Button>
          )}
        ...
        <div className="h-full flex flex-col relative">
          ...
          <div className="flex justify-center items-center p-4 bg-white dark:bg-gray-900 shadow-lg">
            <ThemeSelector
